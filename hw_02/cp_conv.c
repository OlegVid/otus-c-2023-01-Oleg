...
len_of_sym (uint32_t utf_symbol){
  ...
  if (utf_symbol <= 0x7ff)
    return 2;
  if (utf_symbol <= 0xffff)
    return 3;
  if (utf_symbol <= 0x1FFFFFul)
    return 4;
  return 0;
}
...
int64_t cp_to_utf8 (conv_func_t cp_f, uint8_t* inp_buf, int64_t inp_buf_len, uint8_t* outp_buffer, int64_t out_buf_len){
  ...
  for (int64_t i = 0; i < inp_buf_len; i++)
	...
	case 3:
	  //untested !!!
	  outp_buffer[out_length++] =
	    0xE0 | ((uint8_t) (0x0F & (temp_utf_code >> 12)));
	  outp_buffer[out_length++] =
	    0x80 | ((uint8_t) (0x3F & (temp_utf_code >> 6)));
	  outp_buffer[out_length++] =
	    0x80 | ((uint8_t) (0x3F & temp_utf_code));
	  break;
	case 4:
	  //untested !!!
	  outp_buffer[out_length++] =
	    0xF0 | ((uint8_t) (0x07 & (temp_utf_code >> 18)));
	  outp_buffer[out_length++] =
	    0x80 | ((uint8_t) (0x3F & (temp_utf_code >> 12)));
	  outp_buffer[out_length++] =
	    0x80 | ((uint8_t) (0x3F & (temp_utf_code >> 6)));
	  outp_buffer[out_length++] =
	    0x80 | ((uint8_t) (0x3F & temp_utf_code));
