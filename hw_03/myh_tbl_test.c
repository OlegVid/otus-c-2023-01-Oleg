...
int test_myh_tbl_unit(void) {
    ...
    for (size_t i = 0; i < TEST_ITEM_COUNT; i++) {
        ...
        if ((ret_code = myh_table_insert(mht_storage, varName, i * 23)) != MHT_OK) {
            printf("insert error item No = %zu, ret_code = %d \n", i, ret_code);
            return -(i + 1);
        }
#if INJECT_ERROR == 1
